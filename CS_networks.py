...
from turtle import clone, forward
...
#####################################定义综合通道卷积和空间卷积的cat方式##############
###定义PatchEmbed，用于将图像进行分块和嵌入--相当于下采样
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size, input_nc, embed_dim, norm_layer=None):
        super().__init__()
        ...
        self.proj = nn.Conv2d(input_nc, embed_dim, kernel_size=7, stride=patch_size, padding=3)
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
        return x  #(B,3,256,256)->(B,embed_dim,H/patch_size,W/patch_size)->(B,embed_dim,(H/patch_size)*(W/patch_size))->(B,(H/patch_size)*(W/patch_size),embed_dim)
...
class SCBlock(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, img_size, patch_size, mlp_ratio=4, qkv_bias=False, drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, cpe_act=False):
        super().__init__()
        model_sc = []
        model_sc += [PatchEmbed(img_size, patch_size, in_dim, out_dim, norm_layer=None)]
        model_sc += [SpaceBlock(out_dim,num_heads, mlp_ratio,drop_path,
                 act_layer, norm_layer, cpe_act)]
        model_sc += [ChannelBlock(out_dim, num_heads, mlp_ratio, qkv_bias, drop_path,
                 act_layer, norm_layer, cpe_act)]
        self.model_SC = nn.Sequential(*model_sc)
    def forward(self,x):
        x = self.model_SC(x)
        return x
###定义使用SCBloc模块所产生的特征图，用于和上采样cat
class DS_SCBlock(nn.Module):
    def __init__(self,in_dim, out_dim, num_heads, img_size, patch_size, mlp_ratio=4, qkv_bias=False, drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, cpe_act=False):
        super().__init__()
        self.sc1 = SCBlock(in_dim=3, out_dim=256, img_size=256, patch_size=4, mlp_ratio=4)
        self.sc2 = SCBlock(in_dim=3, out_dim=128, img_size=256, patch_size=2)
        self.sc3 = SCBlock(in_dim=3, out_dim=64, img_size=256, patch_size=1)
    def forward(self, x):
        sc1 = self.sc1(x)
        sc2 = self.sc2(x)
        sc3 = self.sc3(x)
        return sc1,sc2,sc3
##################################################
###采用SENet中的通道注意力机制，不用通道transformer 是不是马赛克效应会减少---edit 2022/4/28
###定义SECA模块
class SECA(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.res = nn.Sequential(nn.Conv2d(channel, channel, kernel_size=1, stride=1),
                                 nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1),
                                 nn.Conv2d(channel, channel, kernel_size=1, stride=1),
                                 norm(channel), activation
                                 )
    ...
    def forward(self, x):
        x_res = self.res(x)
        b,c,_,_ = x_res.shape
        y = self.avg_pool(x_res).view(b,c)
        y = self.fc(y).view(b,c,1,1)
        y_CA = x_res*y.expand_as(x_res)
        out = x + y_CA
        return out
    
    
##################################################
###采用通道替换机制，将前层的特征图通道注意值排行前一半的通道替换掉后面特征图的相应通道 ---edit 2022/5/5
##利用SENet得到通道注意值的排行m
class Channel_Sort(nn.Module):
    def __init__(self, channel, reduction=16):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
...
######################梯度差分损失 ---edit 2022/5/7#######################################
class GDL(nn.Module):
    def __init__(self, weight=None, size_average=True):
        super().__init__()
    def forward(self, inputs, targets, lambda_gdl):

        #squared_error = (inputs - targets).pow(2)
        gradient_diff_i = (inputs.diff(axis=-1)-targets.diff(axis=-1)).pow(2) ##在w方向上进行差分，后一项减去前一项
        gradient_diff_j =  (inputs.diff(axis=-2)-targets.diff(axis=-2)).pow(2)
        #loss = (lambda_l1*l1_error + lambda_gdl*gradient_diff_i.sum() + lambda_gdl*gradient_diff_j.sum())/inputs.numel()
        gdl_error = (gradient_diff_i.sum() + gradient_diff_j.sum())/inputs.numel()
        loss = lambda_gdl*gdl_error
        return loss
#############################################################################
#ssim loss
...
class ResTrans(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=2, n_blocks=4, num_heads=16, window_size=16, scales=4):
        ...
        #在上采样前加一层卷积(1,256,64,64)-->(1,256,64,64)为了和下采样对称
        self.model2 = nn.Sequential(nn.Conv2d(mult*ngf, mult*ngf, kernel_size=3, stride=1, padding=1), norm_layer1(mult*ngf), activation)
        ...
        self.model3 = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0), nn.Tanh())
        ...
        for i in range(n_downsampling+1): ## 0 1 2
            ...
            self.concat_dim.append(concat)  ##(512 --> 256) (256 --> 128) (128 --> 64)
        ###SCBlocks###
        self.sc1 = SCBlock(in_dim=3, out_dim=256, num_heads=8, img_size=256, patch_size=4, mlp_ratio=4,)
        self.sc2 = SCBlock(in_dim=3, out_dim=128, num_heads=8, img_size=256, patch_size=2, mlp_ratio=4)
        self.sc3 = SCBlock(in_dim=3, out_dim=64, num_heads=8, img_size=256, patch_size=1, mlp_ratio=4)
        self.sc_decoder = SCBlock(in_dim=128, out_dim=128, num_heads=8, img_size=128, patch_size=1,mlp_ratio=4)
...
#########---Cat1---#############################################################################################
    ...
    def forward_up(self,x, sc3):
        SC = [sc3]
        
        x = self.model2(x)
        #x = torch.cat([x, SC[0]], 1)
        #x = self.concat_dim[0](x)
        for inx, layer_up in enumerate(self.model_U):
            if inx == 0:
                x = layer_up(x)
                #x = torch.cat([x, SC[0]], 1)
                #x = self.concat_dim[inx+1](x)
                #B, C, H, W = x.shape
                #x = self.sc_decoder(x) #(1,128*128,128)
    ...
    def forward(self,x):
        ...
        x = self.model_D(x)
        x = self.model_M(x)
        #sc1 = self.sc1(init) #(1,64*64,256)
        #B1,N1,C1 = sc1.shape
        #sc1 = sc1.view(B1, int(math.sqrt(N1)), int(math.sqrt(N1)), C1).permute(0,3,1,2)
        #sc2 = self.sc2(init) #(1,128*128,128)
        #B2,N2,C2 = sc2.shape
        #sc2 = sc2.view(B2, int(math.sqrt(N2)), int(math.sqrt(N2)), C2).permute(0,3,1,2)
        sc3 = self.sc3(init) #(1,256*256,64)
        B3,N3,C3 = sc3.shape
        sc3 = sc3.view(B3, int(math.sqrt(N3)), int(math.sqrt(N3)), C3).permute(0,3,1,2)
        x = self.forward_up(x, sc3)
        #x = self.model2(x)
        #x1 = self.model_U[0](x)
        #B,C,H,W = x1.shape
        #x1 = self.sc_decoder(x1)
...
class ResTrans_EN(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=2, n_blocks=4, num_heads=16, window_size=16, scales=4):
        ...
        ###channel attention ---edit 2022/4/19
        #self.CB = ChannelBlock(dim=256, num_heads=8)
        ### another channel attention 不使用transformer 采用SENet---edit 2022/4/28
        #self.SECA = SECA(channel=256)
        ###cat后降维（gate方式） ---edit 2022/5/1
        #scale = 2 ** n_downsampling
        #self.dim_d = nn.Conv2d(ngf*scale*2, ngf*scale, kernel_size=1, stride=1)
        ###########通道交换####### ---edit 2022/5/5
        self.channel_sort = Channel_Sort(channel=256)
        
        ###结合256*256*64和128*128*128的低层特征图---edit 2022/4/22---失败
        ###1*1conv+BN
        #scale = 2**n_downsampling
        #self.model3 = nn.Sequential(nn.Conv2d(ngf*7, ngf*scale, kernel_size=1, stride=1), norm_layer1( ngf*scale))
        ###maxpool
        #self.pool = nn.Sequential(nn.MaxPool2d(kernel_size=4), nn.MaxPool2d(kernel_size=2))
################################################################################################################
    ##定义采用多级渐进方式的过度方式 ---2022/4/11   采用channel attention去除冗余特征---2022/4/19
    def multi_feature(self, x, sigmoid):
        x_d1 = self.model_D[0](x)
        x_d2 = self.model_D[1](x_d1)
        ##################################################
        ##channel attention x_d2
        B,C,H,W = x_d2.shape
        x_d2_c = x_d2.permute(0,2,3,1).view(B,-1,C)
        x_d2_c = self.CB(x_d2_c)
        x_d2_c = x_d2_c.view(B,H,W,C).permute(0,3,1,2)
        g_d2_c = sigmoid(x_d2_c)
        ##
        x_m1 = self.model_M[0](x_d2)
        g_m1 = sigmoid(x_m1)
        #x_m1E = (1 + g_m1)*x_m1 + (1 - g_m1)*(g_d2_c * x_d2_c)
        x_m1E = x_m1 + (1 - g_m1)*(g_d2_c * x_d2_c) ##去掉系数
        ####################################################
        ##channel attention x_m1E
        x_m1E_c = x_m1E.permute(0,2,3,1).view(B,-1,C)
        x_m1E_c = self.CB(x_m1E_c)
        x_m1E_c = x_m1E_c.view(B,H,W,C).permute(0,3,1,2)
        g_m1_c = sigmoid(x_m1E_c)
        ##
        x_m2 = self.model_M[1](x_m1E)
        g_m2 = sigmoid(x_m2)
        #x_m2E = (1 + g_m2)*x_m2 + (1 - g_m2)*(g_d2 * x_d2 + g_m1 * x_m1E) #与前面所有层融合
        #x_m2E = (1 + g_m2)*x_m2 + (1 - g_m2)*( g_m1_c * x_m1E_c) #只与前一层融合
        x_m2E = x_m2 + (1 - g_m2)*( g_m1_c * x_m1E_c) ##去掉系数
        #####################################################
        ##channel attention x_m2E
        x_m2E_c = x_m2E.permute(0,2,3,1).view(B,-1,C)
        x_m2E_c = self.CB(x_m2E_c)
        x_m2E_c = x_m2E_c.view(B,H,W,C).permute(0,3,1,2)
        g_m2_c = sigmoid(x_m2E_c)
        ##
        x_m3 = self.model_M[2](x_m2E)
        g_m3 = sigmoid(x_m3)
        #x_m3E = (1 + g_m3)*x_m3 + (1 - g_m3)*(g_d2 * x_d2 + g_m1 * x_m1E + g_m2 * x_m2E) #与前面所有层融合
        #x_m3E = (1 + g_m3)*x_m3 + (1 - g_m3)*( g_m2_c * x_m2E_c) #只与前一层融合
        x_m3E = x_m3 + (1 - g_m3)*( g_m2_c * x_m2E_c) ##去掉系数
        ######################################################
        ##channel attention x_m3E
        x_m3E_c = x_m3E.permute(0,2,3,1).view(B,-1,C)
        x_m3E_c = self.CB(x_m3E_c)
        x_m3E_c = x_m3E_c.view(B,H,W,C).permute(0,3,1,2)
        g_m3_c = sigmoid(x_m3E_c)
        ##
        x_m4 = self.model_M[3](x_m3E)
        g_m4 = sigmoid(x_m4)
        #x_m4E = (1 + g_m4)*x_m4 + (1 - g_m4)*(g_d2 * x_d2 + g_m1 * x_m1E + g_m2 * x_m2E + g_m3 * x_m3E) #与前面所有层融合
        #x_m4E = (1 + g_m4)*x_m4 + (1 - g_m4)*(g_m3_c * x_m3E_c) #只与前一层融合
        x_m4E = x_m4 + (1 - g_m4)*(g_m3_c * x_m3E_c) ##去掉系数
        ######################################################
        ##channel attention x_m4E
        x_m4E = x_m4E.permute(0,2,3,1).view(B,-1,C)
        x_m4E = self.CB(x_m4E)
        x_m4E = x_m4E.view(B,H,W,C).permute(0,3,1,2)
        return x_m4E
        
    ##将256*256*64和128*128*128进行下采样之后与64*64*256cat之后进行1*1conv+BN降维，之后在通过通道注意力机制
    ##定义采用多级渐进方式的过度方式 去掉当前层前面的系数
    def multi_feature_v2(self, x, sigmoid):
        x_init_p = self.pool[0](x)  ##1*64*256*256特征图进行pooling到1*64*64*64
        x_d1 = self.model_D[0](x)
        x_d1_p = self.pool[1](x_d1) ##1*128*128*128特征图进行pooling到1*128*64*64
        x_d2 = self.model_D[1](x_d1) ##1*256*64*64
        x_d2_mix = torch.cat([x_init_p, x_d1_p, x_d2], 1)  ##1*448*64*64
        x_d2_new = self.model3(x_d2_mix)
        ##################################################
        ##channel attention x_d2_new
        B,C,H,W = x_d2.shape
        x_d2_c = x_d2_new.permute(0,2,3,1).view(B,-1,C)
        x_d2_c = self.CB(x_d2_c)
        x_d2_c = x_d2_c.view(B,H,W,C).permute(0,3,1,2)
        g_d2_c = sigmoid(x_d2_c)
        ##
        x_m1 = self.model_M[0](x_d2)
        g_m1 = sigmoid(x_m1)
        x_m1E = x_m1 + (1 - g_m1)*(g_d2_c * x_d2_c)
        ####################################################
        ##channel attention x_m1E
        x_m1E_c = x_m1E.permute(0,2,3,1).view(B,-1,C)
        x_m1E_c = self.CB(x_m1E_c)
        ...
        g_m2 = sigmoid(x_m2)
        #x_m2E = (1 + g_m2)*x_m2 + (1 - g_m2)*(g_d2 * x_d2 + g_m1 * x_m1E) #与前面所有层融合
        x_m2E = x_m2 + (1 - g_m2)*( g_m1_c * x_m1E_c) #只与前一层融合 且去掉当前层前面的系数
        #####################################################
        ##channel attention x_m2E
        x_m2E_c = x_m2E.permute(0,2,3,1).view(B,-1,C)
        x_m2E_c = self.CB(x_m2E_c)
        ...
        g_m3 = sigmoid(x_m3)
        #x_m3E = (1 + g_m3)*x_m3 + (1 - g_m3)*(g_d2 * x_d2 + g_m1 * x_m1E + g_m2 * x_m2E) #与前面所有层融合
        x_m3E = x_m3 + (1 - g_m3)*( g_m2_c * x_m2E_c) #只与前一层融合
        ######################################################
        ##channel attention x_m3E
        x_m3E_c = x_m3E.permute(0,2,3,1).view(B,-1,C)
        x_m3E_c = self.CB(x_m3E_c)
        ...
        g_m4 = sigmoid(x_m4)
        #x_m4E = (1 + g_m4)*x_m4 + (1 - g_m4)*(g_d2 * x_d2 + g_m1 * x_m1E + g_m2 * x_m2E + g_m3 * x_m3E) #与前面所有层融合
        x_m4E = x_m4 + (1 - g_m4)*(g_m3_c * x_m3E_c) #只与前一层融合
        ######################################################
        ##channel attention x_m4E
        #x_m4E = x_m4E.permute(0,2,3,1).view(B,-1,C)
        #x_m4E = self.CB(x_m4E)
        #x_m4E = x_m4E.view(B,H,W,C).permute(0,3,1,2)
        return x_m4E
    ###定义简化的多级渐进融合方式，只融合(x_d2,x_m2)和(x_m1,x_m3)
    def simple_feature(self, x, sigmoid):
        x_d1 = self.model_D[0](x)
        x_d2 = self.model_D[1](x_d1)
        x_m1 = self.model_M[0](x_d2)
        ...
        x_d2_c = x_d2_c.view(B,H,W,C).permute(0,3,1,2)
        g_m2 = sigmoid(x_m2)
        x_m2_E = x_m2 + (1-g_m2)*x_d2_c
        ###########
        x_m3 = self.model_M[2](x_m2_E)
        ###########
        x_m1_c = x_m1.permute(0,2,3,1).view(B,-1,C)
        x_m1_c = self.CB(x_m1_c)
        x_m1_c = x_m1_c.view(B,H,W,C).permute(0,3,1,2)
        g_m3 = sigmoid(x_m3)
        x_m3_E = x_m3 + (1-g_m3)*x_m1_c
        ###########
        x_m4 = self.model_M[3](x_m3_E)
        return x_m4
    
    ###不采用通道transformer，采用SENet中的通道注意力机制-只融合(x_d2,x_m2)和(x_m1,x_m3)---edit 2022/4/28
    def SECA_feature(self, x, sigmoid):
        x_d1 = self.model_D[0](x)
        x_d2 = self.model_D[1](x_d1)
        x_m1 = self.model_M[0](x_d2)
        x_m2 = self.model_M[1](x_m1)
        ###########
        x_d2_c = self.SECA(x_d2)
        g_m2 = sigmoid(x_m2)
        x_m2_E = x_m2 + (1-g_m2)*x_d2_c
        ###########
        x_m3 = self.model_M[2](x_m2_E)
        ###########
        x_m1_c = self.SECA(x_m1)
        g_m3 = sigmoid(x_m3)
        x_m3_E = x_m3 + (1-g_m3)*x_m1_c
        ###########
        x_m4 = self.model_M[3](x_m3_E)
    ...
    ###不采用通道transformer，采用SENet中的通道注意力机制-只融合(x_d2,x_m2)和(x_m1,x_m3) gate 改为cat方式---edit 2022/5/1
    def SECA_feature_cat(self, x):
        x_d1 = self.model_D[0](x)
        x_d2 = self.model_D[1](x_d1)
        x_m1 = self.model_M[0](x_d2)
        x_m2 = self.model_M[1](x_m1)
        ###########
        x_d2_c = self.SECA(x_d2) ##(1,256,64,64)
        x_m2_cat = torch.cat([x_m2, x_d2_c], 1) ##(1,512,64,64)
        x_m2_E = self.dim_d(x_m2_cat) ##(1,256,64,64)
        ###########
        x_m3 = self.model_M[2](x_m2_E)
        ###########
        x_m1_c = self.SECA(x_m1)
        x_m3_cat = torch.cat([x_m3, x_m1_c], 1)
        x_m3_E = self.dim_d(x_m3_cat)
        ###########
        x_m4 = self.model_M[3](x_m3_E)
        return x_m4
    ########################################################################################
    #######通道交互 ---edit2022/5/5############       
    def channel_feature_mix(self,x):
        x_d1 = self.model_D[0](x)
        x_d2 = self.model_D[1](x_d1)
        x_m1 = self.model_M[0](x_d2)
        
        ########用x_d2的部分通道替换x_m1的相应位置通道
        x_d2_m = self.channel_sort(x_d2) ##得到x_d2的通道注意力值排序结果
        x_m1_new = change(x_d2_m, x_d2, x_m1)
        #######用x_m2的部分通道替换x_m3的相应位子通道
        x_m2 = self.model_M[1](x_m1_new)
        x_m3 = self.model_M[2](x_m2)
        x_m2_m = self.channel_sort(x_m2)
        x_m3_new = change(x_m2_m, x_m2, x_m3)
        #######
        x_m4 = self.model_M[3](x_m3_new)
        return x_m4
    ...
    def forward(self,x):
        ...
        #x = self.model_D(x)
        #x = self.model_M(x)
        #x = self.simple_feature(x,sigmoid = nn.Sigmoid())
        #x = self.SECA_feature_cat(x) ## ---edit 2022/5/1
        x = self.channel_feature_mix(x) ## --edit 2022/5/5
        #x = self.model_U[0](x)
        #x = self.model_U[1](x)
        x = self.model_U(x)
