...
class TransDecoder(nn.Module):
    def __init__(self, img_size=256, patch_size=4, input_nc=3, output_nc=3,
        ...
        print("TransG expand initial----depths:{};;drop_path_rate:{};output_nc:{}".format(depths,
        drop_path_rate,output_nc))
        ...
        for i_layer in range(self.num_layers):
            ...
            concat = nn.Conv2d(dim, dim//2, kernel_size=3, stride=1, padding=1)
            if i_layer < 3:
                
                layer_up = nn.Sequential(nn.ConvTranspose2d(dim, dim//2, kernel_size=3,stride=2,padding=1,output_padding=1),
                                        nn.InstanceNorm2d(dim//2), nn.ReLU(True))
                self.layers_up.append(layer_up)
            self.concat_back_dim.append(concat)

        self.norm = norm_layer(self.num_features)
        self.norm_up= nn.InstanceNorm2d(self.embed_dim)
        ...
        if self.final_upsample == "expand_first":
            ...
            ##采用Final_up
            #self.output = Final_up(input_resolution=(img_size//patch_size,img_size//patch_size), dim=embed_dim)
            self.output = nn.ConvTranspose2d(dim, 3, kernel_size=7, stride=4, padding=2, output_padding=1)
            self.tanh = nn.Tanh()
        self.apply(self._init_weights)
    ...
    def forward_features(self, x):
        ...
        hh = int(math.sqrt(HW))
        ww = int(math.sqrt(HW))
        #x = rearrange(x, 'B (HW) C -> B C H W', H = hh, W = ww ) #x (B,C,H,W)
        x = x.view(B, hh, ww, C).permute(0,3,1,2)
        return x, x_downsample
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            x = layer_up(x)
            x1 = x_downsample[2-inx]
            B,HW,C = x1.shape
            hh = int(math.sqrt(HW))
            ww = int(math.sqrt(HW))
            x1 = x1.view(B, hh, ww, C).permute(0,3,1,2)
            x = torch.cat([x,x1],1)
            x = self.concat_back_dim[inx](x)
        x = self.norm_up(x)  # B L C
